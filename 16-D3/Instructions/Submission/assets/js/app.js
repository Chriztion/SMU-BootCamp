//Make function to make the chart/page responsive
function pageResponsive() {

    // Remove and replace if area is not empty on refresh of page
    var svgArea = d3.select("#scatter").select("svg");

    // Clear the area if not empty
    if (!svgArea.empty()) {
        svgArea.remove();
    }
    ...
    // Update tool tip for the circles
    ...
    d3.csv("assets/data/data.csv").then(function(censusData, err) {
        ...
        var textGroup = overallPoints
            .append("text")
            // .abbr to bring the abbreviation of states
            .text(function(d) {
                return d.abbr;
            })
            // Place the abbr in the scales
            .attr("dx", function(d) {
...
d3.select(window).on("resize", pageResponsive);